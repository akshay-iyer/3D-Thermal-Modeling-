...
void findAllPatches(const cv::Mat& image, cv::Size patternSize, vector<vector<cv::Point> >& msers, int delta, float max_var, float min_diversity, double area_threshold)
{
    ...
    //cv::MSER
    //works
    cv:: MSER m1 (delta, minArea, maxArea, max_var, min_diversity, 200, area_threshold, 0.003, 5); // delta = 8, max var = 0.1
    //cv:: MSER::create(delta, minArea, maxArea, max_var, min_diversity, 200, area_threshold, 0.003, 5); // delta = 8, max var = 0.1
#endif
																											  /*
